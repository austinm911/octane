...
import { SsrEditor as ServerSsrEditor } from '../_fixtures/ssr-editor.tsrx?octane-ssr';
...
it('hydrates the server shell, then applies the system scheme and mounts the editor', async () => {
	preferDark();
	const props = { text: 'Hydrated text' };
	const { html } = renderToString(ServerSsrEditor, props);
	...
	const root = hydrateRoot(container, SsrEditor, props);
	try {
		await nextPaint();
		flushSync(() => {});
