...
Initial Mapbox GL JS 3.5+ and MapLibre GL JS 4+ entry points with an authored Octane component layer; MapLibre production WebGL, lifecycle, SSR, and public API smoke coverage are verified while exhaustive upstream parity remains in progress.
...
Scope/evidence last checked: 2026-08-05.

- The token-free MapLibre production browser lane compiles authored source with Vite and verifies construction, controls, overlays, style data, teardown, and remount.
- A token-backed modern Mapbox browser lane and exhaustive pristine/adapted upstream parity lanes remain follow-up work.
